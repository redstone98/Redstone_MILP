function [revisit_time_vector_info, contact_tables, revisit_vectors, satellite_cadence_info] = generate_revisit_table_L2_adjust_G(access_interval_table, row_index_BCD, A_matrix, tau, selected_gs, k_value)
...
% Extract Ground Point Tables
  T = access_interval_table(row_index_BCD,:);
    ...
    errorbar(selected_gs, mean_value(selected_gs), mean_value(selected_gs)-min_value(selected_gs), max_value(selected_gs)-mean_value(selected_gs),'*','LineStyle','none','color','m','MarkerEdgeColor','r','LineWidth',1)
    % legend('Min, Max, Mean Revisit Time Data','Location','southoutside')    
    hold off
    title("Lattice Result (L_2 BCD, tau = " + tau + " sec, k = " + k_value + ")", 'FontSize',12,'FontWeight','bold');
